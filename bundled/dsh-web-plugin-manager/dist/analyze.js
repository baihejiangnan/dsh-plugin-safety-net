 ...
 * prevent.
 */
function isNonPackageSpec(spec) {
...
export function scanImports(filePath) {
    try {
        ...
        for (const match of code.matchAll(statement)) {
            const spec = match[1];
            if (isNonPackageSpec(spec))
                continue;
            if (typeOnly.has(spec))
                continue;
            found.add(spec);
        }
...
function relativeImports(filePath) {
    try {
        ...
        for (const match of code.matchAll(statement)) {
            const spec = match[1];
            if (spec.startsWith('.') && !spec.endsWith('.css') && !spec.endsWith('.json'))
                found.add(spec);
        }
        for (const match of code.matchAll(sideEffect)) {
            const spec = match[1];
 ...
 */
export function scanPackageImports(pkgDir, entry, maxFiles = 200) {
    if (entry === null)
        return [];
    ...
    while (queue.length > 0 && scanned < maxFiles) {
        ...
        scanned += 1;
        for (const spec of scanImports(current))
            bare.add(spec);
        for (const rel of relativeImports(current)) {
            ...
            if (!resolved.startsWith(pkgDir))
                continue; // never leave the package
            seen.add(resolved);
            queue.push(resolved);
        }
    }
 ...
 */
function scanServices(filePath) {
    ...
    try {
        ...
        for (const match of code.matchAll(providePattern))
        ...
        for (const match of code.matchAll(toolPattern))
            tools.add(match[1]);
        ...
        for (const match of code.matchAll(sectionPattern))
...
export function packageEntry(pkgDir, manifest) {
    const exportsField = manifest['exports'];
    const dot = exportsField !== undefined ? exportsField['.'] : undefined;
    const candidates = [
        dot !== undefined && typeof dot === 'object' ? dot['default'] : undefined,
        manifest['main'],
        manifest['module'],
    ];
...
function satisfiesRange(installed, range) {
    ...
    if (spec === '*' || spec === '')
        return true;
    if (spec.startsWith('>='))
        return compareVersions(installed, spec.slice(2)) >= 0;
    if (spec.startsWith('<='))
        return compareVersions(installed, spec.slice(2)) <= 0;
    if (spec.startsWith('>'))
        return compareVersions(installed, spec.slice(1)) > 0;
    if (spec.startsWith('<'))
        return compareVersions(installed, spec.slice(1)) < 0;
    const req = spec.replace(/^[~^=]/, '');
    if (compareVersions(installed, req) < 0)
        return false;
    ...
    if (spec.startsWith('^')) {
        ...
        if (minor > 0)
            return installedMajor === 0 && installedMinor === minor;
...
function providerOf(specifier, providers) {
    const parts = specifier.split('/');
    for (let i = parts.length; i >= 1; i -= 1) {
        let candidate = parts.slice(0, i).join('/');
        if (candidate.startsWith('@') && i === 1)
            continue; // '@scope' alone is not a package
 ...
 */
export function analyzeProfile(profileDir, bundles, patchContent, disabledNames, extraIssues = []) {
    ...
    const providerDirs = new Map();
    const providerManifests = new Map();
    const collectProvider = (dir) => {
        ...
        if (!providerDirs.has(name)) {
            ...
            if (exportsField !== undefined && typeof exportsField === 'object') {
                for (const key of Object.keys(exportsField)) {
                    if (key.startsWith('.'))
                        providers.set(name + key.slice(1), name);
                }
            }
        }
    };
                    ...
                    if (entry.name.startsWith('@')) {
                        for (const scoped of readdirSafe(dir)) {
                            if (scoped.isDirectory() || scoped.isSymbolicLink())
                                collectProvider(join(dir, scoped.name));
                        }
                    }
                    else {
                        collectProvider(dir);
    ...
    if (fallbackModules !== nodeModules)
        scanRoot(fallbackModules);
    ...
    const idRows = new Map(); // duplicate-row-id detection
    ...
    for (const name of Object.keys(deps)) {
        ...
        const dsh = (pkgManifest['dsh'] ?? {});
        const isBundle = dsh?.bundle?.patch !== undefined
    ...
    const regularDepsByPackage = new Map();
    for (const pkg of packages) {
        ...
        const regularDeps = new Set(Object.keys((pkgManifest['dependencies'] ?? {})));
    ...
    for (const name of profileLayerNames) {
        if (!name.startsWith('@deepseek-ai/'))
            continue;
        if (!fallbackLayerNames.has(name))
            continue;
    ...
    for (const pkg of packages) {
        if (pkg.entryPath === null)
            continue;
        const declared = declaredByPackage.get(pkg.name) ?? new Set();
    ...
    for (const pkg of packages) {
        const providerDir = providerDirs.get(pkg.name);
        const pkgManifest = providerDir !== undefined ? providerManifests.get(pkg.name) ?? {} : readManifest(join(nodeModules, pkg.name));
        const peers = (pkgManifest['peerDependencies'] ?? {});
        for (const [peer, range] of Object.entries(peers)) {
            ...
            if (peerVersion === undefined || peerVersion.length === 0)
                continue;
    ...
    const rowTarget = (pkg) => pkg.rowId ?? pkg.name;
...
function topoSort(adjacency, all) {
 ...
 */
export function scanNodeModulesNames(root) {
    const names = new Set();
    try {
        for (const entry of readdirSafe(root)) {
            if (entry.name.startsWith('.'))
