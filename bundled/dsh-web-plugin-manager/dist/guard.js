...
// reaches the verb.
const DSH_PLUGIN_MUTATION = /\bdsh\s+plugin\b(?:\s+--[^\s]+(?:\s+[^\s-][^\s]*)?)*\s+(?:add|install|remove|rm|update|upgrade|uninstall|delete)\b/;
...
const DENIAL_REASON = 'Plugin installation/removal must go through the protected flow: call the plugin_install / plugin_uninstall / '
...
function commandText(exec) {
    if (exec.name !== 'bash' && exec.name !== 'run_code')
        return null;
 ...
 */
function isRawPluginMutation(command) {
    for (const segment of command.split(/[;\n&|]+/)) {
        if (DSH_PLUGIN_MUTATION.test(segment))
            return true;
        if (PM_MUTATION.test(segment) && PROFILE_DIR_MARKER.test(segment))
            return true;
    }
    return false;
}
...
export function createPluginGuard() {
    return (exec) => {
        ...
        if (command === null)
            return undefined;
        if (!isRawPluginMutation(command))
            return undefined;
