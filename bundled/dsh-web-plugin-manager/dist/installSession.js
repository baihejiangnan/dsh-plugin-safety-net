...
export function createInstallSession(spec, cacheDir, scanned) {
    pruneExpiredSessions();
    const key = sessionKey(spec);
    sessions.set(key, { spec, cacheDir, scanned, createdAt: Date.now() });
    return key;
}
export function getInstallSession(spec) {
    pruneExpiredSessions();
    return sessions.get(sessionKey(spec));
}
/** 丢弃会话（用户取消 / 安装继续后不再需要）。 */
export function dropInstallSession(spec) {
 ...
 */
export function filterAnswers(scanned, answers) {
    const out = {};
