...
export function presetsDirPath() {
 ...
 */
export function findSkillRoots(root, maxDepth = 5, limit = 200) {
    const roots = [];
    const walk = (dir, depth) => {
        ...
        for (const entry of entries) {
            if (!entry.isDirectory() || entry.name.startsWith('.') || entry.name === 'node_modules')
                continue;
            if (VENDORED_DIR_NAMES.has(entry.name.toLowerCase()))
                continue;
...
export function findPresetRoots(root, maxDepth = 3, limit = 50) {
    const roots = [];
    const walk = (dir, depth) => {
        if (roots.length >= limit)
            return;
...
export function findPluginRoots(root, maxDepth = 3, limit = 50) {
    const roots = [];
    const walk = (dir, depth) => {
        if (roots.length >= limit)
            return;
...
export function detectRepoType(root) {
    ...
    if (findPresetRoots(root, 0, 1).length > 0)
        return 'agent-preset';
    if (has('package.json')) {
        let looksLike = false;
        try {
        ...
        if (findSkillRoots(root, 0, 1).length > 0)
            return 'skill';
    ...
    if (findPresetRoots(root).length > 0)
        return 'agent-preset';
    if (findPluginRoots(root).length > 0)
        return 'cordis-plugin';
    if (findSkillRoots(root, 5, 1).length > 0)
        return 'skill';
    return 'instructions';
}
/** Read the SKILL.md display name from its YAML frontmatter; null when absent. */
function skillDisplayName(root) {
...
function copyFilter(src) {
    if (src.split(sep).includes('.git'))
        return false;
    if (src.split(sep).includes('node_modules'))
        return false;
    return true;
}
/**
 ...
 */
export function installSkill(root, repoName, occupied) {
 ...
 * under the repo name.
 */
export function installPreset(root, repoName, occupied) {
    const presetRoots = findPresetRoots(root);
    if (presetRoots.length === 0)
        throw new Error('no agent.cordis.yml found in the repository');
    const repoSlug = slugDirName(repoName.split('/').pop() ?? repoName);
...
let kindRecordsCache = null;
let kindQueue = Promise.resolve();
function enqueueKind(task) {
...
export async function loadKindRecords() {
    if (kindRecordsCache !== null)
        return kindRecordsCache;
    try {
        const data = JSON.parse(readFileSync(kindRecordsFile(), 'utf8'));
...
function writeKindRecords(records) {
...
function kindRecordAlive(record) {
...
export async function loadBlockedRepos() {
    if (blockedCache !== null)
        return blockedCache;
    try {
        const data = JSON.parse(readFileSync(blockedReposFile(), 'utf8'));
        blockedCache = new Set((data.repos ?? []).map((repo) => normalizeRepoRef(repo) ?? repo));
    }
    catch {
...
function writeBlockedRepos(repos) {
...
export function isUnderRoot(target, root) {
    ...
    if (process.platform === 'win32')
        return t.toLowerCase().startsWith(r.toLowerCase() + sep);
 ...
 */
export function rmRetry(target) {
