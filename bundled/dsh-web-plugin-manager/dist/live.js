    ...
    for (const row of clone) {
        if (isRecord(row) && Array.isArray(row.insert)) {
            for (const child of row.insert) {
                if (isRecord(child) && child.disabled === true)
...
let liveApplyTail = Promise.resolve();
...
function includeEntry(ctx) {
    const loader = ctx.get('loader');
    if (loader === undefined)
        return undefined;
    for (const candidate of loader.entries()) {
        if (candidate.id !== 'include')
            continue;
 ...
 */
function scrubBakedDisabled(stack, id) {
...
export async function applyLiveOps(ctx, ops, timeoutMs = 5_000) {
    ...
    for (const op of ops) {
        switch (op.kind) {
            ...
            case 'remove-first': {
                ...
                if (op.id !== undefined)
                    scrubBakedDisabled(stack, op.id);
                break;
            }
            case 'replace-last': {
                let index = -1;
                ...
                if (index < 0)
                    break;
                const next = op.mutate(stack[index]);
                if (next === null)
                    stack.splice(index, 1);
                else
                    stack[index] = next;
...
async function loadYaml() {
    ...
    try {
        ...
        try {
            ...
            const mod = await import(__rewriteRelativeImportExtension(pathToFileURL(candidate).href));
            yamlModule = mod;
            return yamlModule;
        }
        catch {
 ...
 * HMR's lifecycle.
 */
export function ensurePatchWatcher(ctx, patchPath) {
    const dirPath = dirname(patchPath);
    if (patchWatcher !== undefined) {
        if (patchWatcher.patchPath === patchPath && patchWatcher.ctx === ctx)
            return;
        patchWatcher.watcher?.close();
        if (patchWatcher.timer !== undefined)
            clearTimeout(patchWatcher.timer);
    ...
    void (async () => {
        ...
        if (yaml === null || state.others !== undefined)
            return;
        ...
        try {
            ...
            if (Array.isArray(rows) && Array.isArray(stack)) {
                const needle = new Set(normalizeForCompare(rows).map((row) => JSON.stringify(row)));
                state.others = normalizeForCompare(stack).filter((row) => !needle.has(JSON.stringify(row)));
            }
        }
    ...
    try {
        ...
        state.watcher = watch(dirPath, (_event, filename) => {
            if (typeof filename !== 'string' || filename !== basename(patchPath))
                return;
            if (state.scheduled)
                return;
 ...
 */
async function recomposeFromFile(state) {
    try {
        const yaml = await loadYaml();
        if (yaml === null)
            return;
