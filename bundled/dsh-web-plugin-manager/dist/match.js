...
export function scoreItem(item, tokens) {
    ...
    for (const token of tokens) {
        if (name.includes(token) || fullName.includes(token))
            score += 3;
...
export function isGitSourceSpec(source) {
    const spec = source.trim();
    if (spec.startsWith('github:') || spec.startsWith('git+') || spec.startsWith('git@'))
        return true;
 ...
 *   退回 @latest（与旧行为一致）。
 */
export function updateSpec(source, name, latest) {
