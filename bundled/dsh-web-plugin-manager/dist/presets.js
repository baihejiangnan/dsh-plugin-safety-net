...
import { dshHomePath, isUnderRoot, presetsDirPath, renameRetry, rmRetry } from "./kinds.js";
...
export function readPresetOwners(presetDir) {
    ...
    for (const file of entries) {
        if (file === OWNER_MARKER || !/owner\.json$/i.test(file))
            continue;
...
export function agentPresetsOf(ctx) {
    try {
        const service = ctx.get?.('agentPresets');
        if (service === undefined || typeof service !== 'object' ||
 ...
 * reported.
 */
export async function cleanupOwnedPresets(ctx, root, pluginName) {
    const removed = [];
    const skipped = [];
 ...
 */
export function archiveOwnedPresets(root, pluginName) {
 ...
 * and is reported.
 */
export function restoreArchivedPresets(root, pluginName) {
    const archiveRoot = presetArchiveDir();
    const restored = [];
    const skipped = [];
 ...
 */
export function pluginInstalledInOtherProfiles(profile, pluginName) {
 ...
 */
export function writeOwnerMarker(presetDir, owners) {
    const marker = join(presetDir, OWNER_MARKER);
    if (existsSync(marker))
        return;
...
export function formatCleanupResult(pluginName, result) {
    ...
    if (result.removed.length > 0)
        parts.push('removed ' + result.removed.length + ' preset(s): ' + result.removed.join(', '));
...
export function formatArchiveResult(pluginName, result) {
    ...
    if (result.archived.length > 0)
        parts.push('moved ' + result.archived.join(', ') + ' out of the preset picker');
    for (const skip of result.skipped)
        parts.push('kept ' + skip.id + ' (' + skip.reason + ')');
    return parts.length > 0
...
export function formatRestoreResult(pluginName, result) {
    ...
    if (result.restored.length > 0)
        parts.push('restored ' + result.restored.join(', ') + ' to the preset picker');
    for (const skip of result.skipped)
        parts.push('kept archived ' + skip.id + ' (' + skip.reason + ')');
    return parts.length > 0
