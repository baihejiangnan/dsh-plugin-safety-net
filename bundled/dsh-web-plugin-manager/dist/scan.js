 ...
 */
export async function scanRequirements(dir) {
    const names = new Set();
    const files = [];
    const walk = async (current, depth) => {
        if (depth > SCAN_MAX_DEPTH || files.length >= SCAN_MAX_FILES)
            return;
        let entries;
        try {
    ...
    for (const file of files.slice(0, SCAN_MAX_FILES)) {
        try {
            ...
            for (const m of text.matchAll(ENV_PATTERN))
                names.add(m[0]);
        }
        catch {
 ...
 */
export function buildFilteredEnv(base = process.env) {
    const env = {};
