...
function renderRows(_args, value) {
...
export function createPluginTools(deps) {
    const statusRows = () => {
        ...
        for (const entry of snap.entries) {
            if (rows.some(r => r.id === entry.entryId || r.name === entry.moduleName))
                continue;
            rows.push({
                id: entry.entryId,
                kind: 'entry',
    ...
    return [
        ...
        defineTool({
            ...
            async execute(args) {
                ...
                if (source === '')
                    throw new Error('plugin_install: source must be a non-empty package name');
                const result = await deps.install(source);
                if (!result.ok)
                    throw new Error(`plugin_install failed: ${result.output.slice(0, 2000)}`);
        ...
        defineTool({
            ...
            async execute(args) {
                ...
                if (id === '')
                    throw new Error('plugin_uninstall: id must be a non-empty plugin id');
                // Try the managed insert-row shape first (live removal).
                const insert = await deps.removeInsert(id);
                if (insert.ok)
                    return { ok: true, message: insert.message };
        ...
        defineTool({
            ...
            async execute(args) {
                ...
                const snap = deps.snapshot();
                const entry = snap.entries.find(e => e.entryId === entryId || e.moduleName === entryId);
                if (entry === undefined) {
                ...
                if (!result.ok)
                    throw new Error(`plugin_toggle failed: ${result.message}`);
                return {
...
export function registerTools(ctx, host, profile) {
